...
//    // All state is set atomically through configure(), so we use private vars
//    // and never expose didSet side-effects that could trigger double redraws.
//
...
//    }
//
//    // MARK: - Configure  ← the single "prepare" entry-point, no closures
//    //
//    // rangeStart : the midnight of the first day visible in the calendar
//    //              (Sunday of the week, or 1st of the month)
//    // moodLogs   : already-filtered logs for that range, sorted by date
//    // isWeekly   : true → 7-column chart  /  false → 30-column chart
//
...
//    // MARK: - Data
//    //
//    // KEY CHANGE: dayDates are now built forward from _rangeStart,
//    // NOT backwards from today.  This means if the user is viewing
//    // the week of Dec 28–Jan 3 or the month of March, the x-axis
//    // labels and data points match exactly what's on the calendar.
//
//    private func setData() {
//        let totalDays = _isWeekly ? 7 : 30
//        let calendar  = Calendar.current
//
//        // ── Step 1: Build day dates forward from rangeStart ───────────────────
//        let dayDates: [Date] = (0..<totalDays).compactMap { offset in
//            calendar.date(byAdding: .day, value: offset, to: _rangeStart)
//        }
//
//        // ── Step 2: Labels — "Mon/Tue…" for weekly, "1/2/3…" for monthly ─────
//        let dateFormatter        = DateFormatter()
//        dateFormatter.dateFormat = _isWeekly ? "EEE" : "d"
//        let dayLabels            = dayDates.map { dateFormatter.string(from: $0) }
//
//        // ── Step 3: Configure X-axis ──────────────────────────────────────────
//        let xAxis            = moodChartView.xAxis
//        xAxis.valueFormatter = IndexAxisValueFormatter(values: dayLabels)
//        xAxis.axisMinimum    = -0.5
//        xAxis.axisMaximum    = Double(totalDays) - 0.5
...
//        }
//
//        // ── Step 5: Group logs by day index within this range ─────────────────
//        var perDayValues: [Int: [(Double, MoodLog)]] = [:]
...
//        }
//
//        // ── Step 6: Build entries + x → MoodLog map ───────────────────────────
//        var entries: [(ChartDataEntry, MoodLog)] = []
//
//        for (dayIndex, pairs) in perDayValues {
//            let n = pairs.count
